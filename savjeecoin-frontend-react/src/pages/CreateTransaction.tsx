...
export function CreateTransaction() {
  ...
  return (
    <>
      ...
      <input disabled type='text' id='from-address' className='w-full border-1 border-gray-400 rounded p-2 mb-8' value={walletKey} />

      <label htmlFor="to-address">To Address</label>
      <input id='to-address' className='w-full border-1 border-gray-400 rounded p-2 mb-8' value={toAddress} onChange={(e) => { setToaddress(e.target.value) }} />

      <label htmlFor="amount">Amount</label>
      <input type="number" id='amount' className='w-full border-1 border-gray-400 rounded p-2 mb-8' value={amount} onChange={(e) => { setAmount(e.target.value) }} />

      <button className='mt-8 font-bold outline-2 bg-blue-500 text-white cursor-pointer p-3 rounded hover:bg-blue-600 active:bg-blue-700' onClick={() => {
        addTransaction()
